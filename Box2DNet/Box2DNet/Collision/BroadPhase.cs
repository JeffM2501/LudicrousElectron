...
/*#warning "CAS"*/
	...
	public class BroadPhase
	{
		...
		public BroadPhase(AABB worldAABB, PairCallback callback)
		{
			...
			Box2DNetDebug.Assert(worldAABB.IsValid);
		...
		{
			Box2DNetDebug.Assert(_proxyCount < Settings.MaxProxies);
			Box2DNetDebug.Assert(_freeProxy != PairManager.NullProxy);
...
/*#warning "Check this"*/
			...
			Box2DNetDebug.Assert(_queryResultCount < Settings.MaxProxies);

			// Create pairs if the AABB is in range.
			for (int i = 0; i < _queryResultCount; ++i)
			{
				Box2DNetDebug.Assert(_queryResults[i] < Settings.MaxProxies);
				Box2DNetDebug.Assert(_proxyPool[_queryResults[i]].IsValid);
		...
		{
			Box2DNetDebug.Assert(0 < _proxyCount && _proxyCount <= Settings.MaxProxies);
			Proxy proxy = _proxyPool[proxyId];
			Box2DNetDebug.Assert(proxy.IsValid);
...
/*#warning "Check this"*/
		...
		int QuerySegment(Segment segment, object[] userData, int maxCount, SortKeyFunc sortKey)
		{
			...
			Box2DNetDebug.Assert(sx != 0 || sy != 0);
			...
			for (int i = 0; i < _queryResultCount && count < maxCount; ++i, ++count)
			{
				...
				Box2DNetDebug.Assert(proxy_.IsValid);
		...
		public void Validate()
		{
			for (int axis = 0; axis < 2; ++axis)
			{
				...
				for (int i = 0; i < boundCount; ++i)
				{
					...
					Box2DNetDebug.Assert(i == 0 || bounds[i - 1].Value <= bound.Value);
					Box2DNetDebug.Assert(bound.ProxyId != PairManager.NullProxy);
					Box2DNetDebug.Assert(_proxyPool[bound.ProxyId].IsValid);

					if (bound.IsLower == true)
					{
						Box2DNetDebug.Assert(_proxyPool[bound.ProxyId].LowerBounds[axis] == i);
						++stabbingCount;
					}
					else
					{
						Box2DNetDebug.Assert(_proxyPool[bound.ProxyId].UpperBounds[axis] == i);
						--stabbingCount;
					}

					Box2DNetDebug.Assert(bound.StabbingCount == stabbingCount);
				}
			}
		}
		...
		internal bool TestOverlap(Proxy p1, Proxy p2)
		{
			for (int axis = 0; axis < 2; ++axis)
			{
				...
				Box2DNetDebug.Assert(p1.LowerBounds[axis] < 2 * _proxyCount);
				Box2DNetDebug.Assert(p1.UpperBounds[axis] < 2 * _proxyCount);
				Box2DNetDebug.Assert(p2.LowerBounds[axis] < 2 * _proxyCount);
				Box2DNetDebug.Assert(p2.UpperBounds[axis] < 2 * _proxyCount);
		...
		internal bool TestOverlap(BoundValues b, Proxy p)
		{
			for (int axis = 0; axis < 2; ++axis)
			{
				...
				Box2DNetDebug.Assert(p.LowerBounds[axis] < 2 * _proxyCount);
				Box2DNetDebug.Assert(p.UpperBounds[axis] < 2 * _proxyCount);
